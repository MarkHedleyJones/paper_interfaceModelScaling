...

{
    ...
    In this manuscript we use an adapted version of the compact non-linear model presented in \cite{ScottSingle2013}. The model is compact in that it is suitable for entering into SPICE, i.e., freely available electrical circuit simulation software. The schematic for the model used here is presented in Figure \ref{fig:schematic}.
}
...
The electrical impedance between two electrodes in an electrolyte arises from two interface impedances in series with the resistance due to the bulk of the electrolyte itself. To quantify the impedance presented by a single interface we first need to determine inter-electrode resistance presented by the electrolyte bulk. {\color{blue} Finding these resistances allows us to quantify the interface's impedance via subtraction.} As there are always two interface impedances between any two electrodes, inter-electrode resistances cannot be measured in isolation.

In \cite{ScottSingle2013}, a series of transresistance measurements were used in conjunction with a physical description of the electrode geometry to build up a representative network of resistors. That resistor network was defined using five parameters: $R_{eri}$, $R_{lri}$, $R_{li}$, mesh depth, and edge depth. Readers are referred to \cite{ScottSingle2013} for the interpretation of these model parameters. Repeating that work, we have created a network of resistors that mimic the resistances due to the solution's bulk conductivity.

The mesh depth and padding values of five columns and three rows, respectively, has been taken directly from \cite{ScottSingle2013}. {\color{blue} These parameters, together with an array of 8 electrodes, leads to a generated network of 205 resistors, the layout of which is presented as figure \ref{fig:mesh}.}

A numerical fit is then made to the two independent scaling parameters $R_{eri}$ and $R_{li}$
...
Measurements of the inter-electrode impedance are made using a Tektronix TPS2014 DSO with four fully isolated and floating channels; an Agilent 33220A Function Generator; and a desktop PC {\color{blue} running Python 2.7 for instrumentation control and result processing.} The pH level of the stock solution is measured using a calibrated EDT Instruments BA-350 pH meter. It has a pH of 7.4 and no pH adjustments were made to the derived solutions.


{
    \color{blue}
    To be clear, the resistive mesh created in this step models \textit{only} the resistivity of the solution bulk. No contribution from the interface itself appears in these measurements. The is because voltages are always measured across pairs of non-driven electrodes and the measurements are of suitably high impedance. We determine the resistive component of the interface, $R_{S}$, in the next section.
}
...
\begin{figure}
    ...
    \end{center}
    \caption{\color{blue} Per phase charge injected during CPE measurement in each concentrations of PBS, calculated using an effective surface area of 14\thinspace mm$^{2}$. {\color{red} IS THIS CHEATING? (I have given the charged injected in terms of effective surface area instead of geometric)}}
...
Possible irreversible Faradaic reactions for platinum electrodes {\color{blue}(each with a different reaction potential)} in saline are:
...
We measure Faradaic conduction by separating it from the displacement behaviour of the CPE. By using a DC measurement technique, as opposed to cyclic measurements, we are able to separate the effects in time.
...
{\color{blue}
    Four measurements of displacement current (CPE element in the model) were} taken at regular intervals over a period of 30 minutes and have been presented in figures~\ref{fig:displacement_sheepCPEMagnitude} and~\ref{fig:displacement_sheepCPEPhase}.
...
Using DC measurements we conclude that the {\color{blue} concentration of PBS has no effect on the onset of Faradaic conduction for the range of concentrations investigated.} We hypothesise that this is because the double-layer sets the effective concentration of ions{\color{blue}, those involved in the Faradaic reactions,} in a localised volume surrounding each electrode.
...

{
    ...
    The final model, including 205 lines of resistive mesh, is represented by a SPICE netlist comprised of 348 lines. Simulation of this model, using the freely available ngSpice circuit simulator, at 1000 frequencies between spaced between 50\thinspace mHz to 10\thinspace kHz takes approximately 1 second on a modern CPU. 
}
{
    \color{red}
    How is this last line? It's new information in the conclusion...
}

\section*{Acknowledgment}
The authors wish to acknowledge the The University of Waikato for financial support and Saluda Medical for the use of facilities and equipment. {\color{red}Special thanks are expressed to Peter Single of Saluda Medical for help conducting in-vivo measurements.}
...
{\color{OliveGreen}
    ...
    {
        \color{blue}
        The diodes are there to model the change in current attributed to the first biologically damaging reaction that occurs at the electrode/electrolyte interface. Whatever reaction occurs first in the relevant electrolyte is the reaction that diode aims to mimic. We observe that the current increases near exponentially around this point so we use a diode to mimic that. We expect this reaction may not be the same in-vivo as it is in-vivo but the properties of a diode can be altered to match the situation.
    ...
    {
        \color{blue}
        This is a good point. ScottSingle2013 compare the models prediction to stimulus waveforms. Our aim is to capture how the model parameters change as the concentration of PBS is varied and to compare those changes to parameters obtained from in-vivo measurements using a live sheep. Were doing this because 0.1X PBS is commonly used as a phantom for real spine. There hasn't been an objective way of determining how accurate that assumption was until we used the model to make this comparison. In future we hope to match it against in-vivo stimulus waveforms; at present it is out of scrope.
    }
    ...
    {
        \color{blue}
        That is interesting and definitely important to our modelling work. We had presented the charge injected in terms of the geometric surface area of each electrode which we know is less than the real (or effective) surface area, estimated to be $14\thinspace mm^{2}$. Calculating the charge injection using the effective surface area is more relevant here and gives results consistant with Brummer \& Roblee (Annals New York Academy of Sciences, 159-171, 1983), who also express their values in terms of ``real'' area. 

    }
    {
        \color{red}
        How does this read now? In your last comment you said...

        We have to acknowledge that this is anomalous. We were stirring, but definitely observed no gas. Say just as you say, the results are consistent with it NOT being a problem, although other researchers have observed otherwise. We seem to be saying that they are wrong, so we should say just that, we observe something different, yet results are clearly consistent and explicable!
    }
...

{
    ...
    {
        \color{blue}
        The 0.9V limitation is rather arbitrary as it was specific to the use of electrodes 2 and 7. We have instead clarified that the model only captures the onset of Faradaic conduction sufficient for it to be avoided. A sentence to this effect have been added to the end of the conclusion. 
    }
}
